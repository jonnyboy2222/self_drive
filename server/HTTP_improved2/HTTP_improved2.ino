...
void setupCamera() {
    ...
    if (err != ESP_OK) {
        // Serial.print("FATAL ERROR: Camera init failed with error ");
        // Serial.print(err);
    ...
    while (true) {
        if (Serial1.available()) {
            ...
            if (len > 0) {
                ...
                // Ensure data is not empty and fits within the queue item size
                if (sensorDataStr.length() > 0 && sensorDataStr.length() < SENSOR_DATA_QUEUE_ITEM_SIZE) {
                    // Serial.println("UART Received: " + sensorDataStr);

                    // --- Queue the data for the Network Task ---
                    // 1. Copy the received data into the fixed-size buffer
                    ...
                    // 2. Send a *copy* of the data buffer to the queue.
                ...
                } else if (sensorDataStr.length() >= SENSOR_DATA_QUEUE_ITEM_SIZE) {
                    // Serial.println("UART Error: Received data exceeds queue item size! Discarding.");
                }
            } else {
...
// =========================================================================
// RTOS TASK: Network Send (POST sensor data to Flask) - MODIFIED
// =========================================================================
void networkSendTask(void *parameter) {
    ...
    while (true) {
        ...
        if (xQueueReceive(sensorDataQueue, dataToPost, portMAX_DELAY) == pdPASS) {
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                // Perform the blocking HTTP POST request using the data from the buffer.
                ...
                if (httpResponseCode > 0) {
                    ...
                    // Serial.print("Network Task: POST successful, Response Code: ");
                    // Serial.println(httpResponseCode);
                    // Serial.println("Flask Response: " + response);
                } else {
                    // Serial.print("Network Task: POST failed, Error Code: ");
                    // Serial.println(httpResponseCode);
