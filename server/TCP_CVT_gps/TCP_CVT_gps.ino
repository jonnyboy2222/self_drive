...
IPAddress staticIP(172, 20, 10, 3); // Should be outside the DHCP range (pick 20 <= num or 200 <= num)
IPAddress gateway(172, 20, 10, 1);    // Router's IP (Gateway)
IPAddress subnet(255, 255, 255, 240);   // Standard subnet mask

// --- Python TCP Server Configuration (from tcp.py) ---
const char* python_server_ip = "172.20.10.6";
            ...
            if (len > 0) {
                ...
                if (sensorDataStr.length() > 0) {
                    ...
                    if (!error) {
                        ...
                        else if (strcmp(purpose, "alert") == 0) {
                            char sensorType[32];
                            strncpy(sensorType, doc["sensor_type"] | "", sizeof(sensorType) - 1);
                            sensorType[sizeof(sensorType) - 1] = '\0';
                            if (strlen(sensorType) > 0) {
                                if (xQueueSend(gpsRequestQueue, sensorType, pdMS_TO_TICKS(10)) != pdPASS) {
                                    // Serial.println("UART Warn: Failed to queue GPS request");
                                }
                            }
                        }
                    }
...
// =========================================================================
void sendDataOverTCP(WiFiClient& client, const char* data, const int retryDelay) {
    if (client.connected()) {
        if (!client.println(data)) {
            client.stop(); // 전송 실패 시 연결 종료
            vTaskDelay(retryDelay / portTICK_PERIOD_MS);
        }
    } else {
        client.stop(); // 보낼 때 연결이 없다면 종료 후 재연결 유도
    }
}
...
void networkSendTask(void *parameter) {
    ...
    char gpsData[256];
    const int TCP_RECONNECT_DELAY_MS = 3000;
    ...
    while (true) {
        ...
        // SENSOR 데이터 우선 처리
        if (xQueueReceive(sensorDataQueue, dataToPost, pdMS_TO_TICKS(100)) == pdPASS) {
            sendDataOverTCP(client, dataToPost, TCP_RECONNECT_DELAY_MS);
        }

        // GPS 데이터 처리
        if (xQueueReceive(gpsDataQueue, gpsData, pdMS_TO_TICKS(100)) == pdPASS) {
            sendDataOverTCP(client, gpsData, TCP_RECONNECT_DELAY_MS);
        }
...
// =========================================================================
// RTOS TASK: Alert Send (send gps with sensor value to pc)
// =========================================================================
void updateGPS() {
...
void gpsAttachTask(void *pvParameters) {
    char sensorType[32];

    while (true) {
        if (xQueueReceive(gpsRequestQueue, &sensorType, portMAX_DELAY)) {
            updateGPS();
            if (gps.location.isValid() && gps.location.isUpdated()) {
                StaticJsonDocument<256> alertDoc;
                alertDoc["purpose"] = "alert";
                alertDoc["sensor_type"] = sensorType;

                JsonArray gpsArray = alertDoc.createNestedArray("gps");
                gpsArray.add(gps.location.lat());
                gpsArray.add(gps.location.lng());

                char gpsPayload[256];
                serializeJson(alertDoc, gpsPayload, sizeof(gpsPayload));

                xQueueSend(gpsDataQueue, gpsPayload, pdMS_TO_TICKS(10));
            } else {
...
void setup() {
    ...
    gpsRequestQueue = xQueueCreate(5, sizeof(char) * 32);
    gpsDataQueue = xQueueCreate(5, sizeof(char) * 256);
